...
def parse_args():
    ...
    parser.add_argument('--remark', type=str, default='normal', help='exp remark')
...
def test(model, loader, num_class=40):
    ...
    for j, data in tqdm(enumerate(loader), total=len(loader)):
        points, target = data
        # target = target[:, 0]
        # print("Point",points.shape)  [24, 1024, 6]
        points = points.transpose(2, 1)  #转置的原因是为了pointnet第一位STN时候以为卷积由6channel变为后面的特征channel,
        # 符合conv1d 的操作，[batch, width, channel]
        # print("Point_transpose",points.shape) [24, 6, 1024]
